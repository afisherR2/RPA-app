...
RWC_group <- function(.data, .dr, .sdate, .edate) { # .data = cleaned dmr, .dr = dilution ratio, .sdate = dateslider input 1, .edate = dateslider input 2
  .data |> 
...
summary_panel <- tabsetPanel(
  ...
  tabPanel('sstats',
           ...
           sliderInput('dateSlider', h5('Date Range:'),
                       min = as.Date(today() %m-% years(5), '%Y-%m-%d'),
                       max = as.Date(today(), '%Y-%m-%d'),
                       value = c(as.Date(today() %m-% years(5), '%Y-%m-%d'), as.Date(today(), '%Y-%m-%d')),
                       step = 365,
                       ticks = FALSE,
                       timeFormat = '%Y')
...
# change dmr value to numeric and monitoring period to date
    ...
    mpstats <- eventReactive(input$nextBtn2, {
      
      RWC_group(.data = dmr_of(), .dr = 1, .sdate = min(dmr_of()$monitoring_period_end_date), .edate = max(dmr_of()$monitoring_period_end_date))
    ...
    observeEvent(input$DR, {
      ...
      mpstats <- RWC_group(.data = dmr_of(), .dr = input$DR, .sdate = min(dmr_of()$monitoring_period_end_date), .edate = max(dmr_of()$monitoring_period_end_date))
...
#####
        ...
        paramtab <- reactiveValues(
          nparam = sort(unique(mpstats()$parameter_desc))) # list of parameters
...
# Summary Stats csv download ----------------------------------------------------------
                                # br(),
                                  ...
                                  #               
                                  #               content = function(file) {
                                  #                 write.csv(mpstats(), file, row.names = FALSE)
                                  #               },
                                  #               
